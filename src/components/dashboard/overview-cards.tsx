...
export function OverviewCards({ kpis }: OverviewCardsProps) {
  ...
  return (
    <div className="grid gap-6 md:grid-cols-3">
      ...
      <Card className="bg-card text-dark border-black/10 group">
        <CardHeader className="flex flex-row items-center justify-between pb-2">
          <CardTitle className="text-sm font-medium opacity-50 uppercase tracking-widest font-body">Liderança</CardTitle>
          <MapPin className="h-4 w-4 text-accent transition-transform group-hover:scale-125" />
        </CardHeader>
        <CardContent>
          <div className="text-3xl md:text-4xl font-display mt-2">{topModality}</div>
          <p className="text-xs text-gray-500 mt-2 font-body font-light">Modalidade mais recorrente</p>
        </CardContent>
      </Card>
    </div>
  );
